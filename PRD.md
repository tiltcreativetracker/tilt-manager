...
A shared reference surface for the team's live editing-style Notion page. One PM/admin pastes the **published** notion.site URL into the input at the top; the page renders below in a sandboxed iframe. The URL is persisted in `STATE.editingStyleNotionUrl` and synced through the normal Firestore snapshot so every teammate sees the same page on next render. Viewers see the embed and an "Open in new tab" fallback but no input.

- **Scheme allowlist (security boundary)** — `isSafeEmbedUrl(url)` only accepts `http://` / `https://` URLs. Enforced at both the write side (`App.setEditingStyleUrl` toasts and rejects anything else) and the read side (`renderEditingStyleView` blanks the iframe/link if a legacy `javascript:` / `data:` / `vbscript:` URL is already in Firestore, and surfaces a red "stored URL was rejected" message so an admin can clear or replace it). Without this, a stored `javascript:` URL would execute in the parent origin — the iframe carries `sandbox="allow-scripts allow-same-origin allow-popups allow-forms allow-popups-to-escape-sandbox"` for the sake of Notion.
- **Notion caveat** — only pages published via *Share → Publish → Publish to web* (notion.site URLs) can be iframed; private notion.so URLs are blocked by X-Frame-Options and render empty. The fallback copy points to *Open in new tab*.
...
- **Role visibility** — visible to every role including viewer (`VIEWER_TABS`). Only `roleAtLeast('pm')` sees the input; the `canEdit` check fails **closed** if `roleAtLeast` isn't defined (previously fell open — see §9.5 · 2026-09-12).
