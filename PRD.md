...
- `STATE.trainingCompletions{}` — keyed by editor email: `{ [moduleId]: { startedAt, completedAt, submissionUrls: [str,…], (legacy submissionUrl) } }`. Aliased emails (Sharm has `sharm` and `sharmaine`) merge on read via `_editorTrainingCompletions`.
...
### 5.12 Training *(added 2026-09-13 Batch E, restyled 2026-09-14 → 2026-09-15)*

Replaces the pull-based training-module CRUD. Editors self-pick briefs when idle: title + description, a **Task Brief** (Notion) link, a **GDrive** walkthrough link (was named `loomUrl` — the field key is preserved for back-compat; the label reads "GDrive"), an optional **Assets** (raw footage) link, and one or more **Frame.io submission** slots. Data lives in `STATE.trainingModules[]` (persisted in the main snapshot doc) and `STATE.trainingCompletions[<email>][<moduleId>] = { startedAt, completedAt, submissionUrls: [] }`.

- **Multiple submissions per module** — `requiredSubmissions` on the module (default 1, max 20) defines how many Frame.io URLs the editor needs to hand in. Editors see per-slot inputs; filled slots collapse into a `Frame ↗` link with edit/clear pencils. Legacy single-string `submissionUrl` is auto-promoted into slot 0 on read, so old modules migrate cleanly.
- **Editor view** — the list hides modules with no working content links (an empty module is nothing for an editor to do). Each card shows Not started / In progress / ✓ Completed pills + Start / Mark complete / Undo pill buttons. **GDrive walkthroughs and raw-footage links that resolve to a Drive file URL render as inline `<iframe>` previews** via `driveEmbedUrl()` (normalises `/view`, `open?id=`, `uc?id=` to `/preview`); folder links stay as plain links because Drive can't embed folders.
- **Admin / Content Lead view** — the same editor cards, but each card carries a **per-editor completion strip** (chips for Zidni / Sharm / Patty / Elsa keyed via `EDITOR_EMAILS` aliases so `sharm@` and `sharmaine@` both resolve to Sharm) with `→ <Editor>` **Send to editor threads** buttons that post the training brief straight into that editor's daily Slack thread via `sendSlackChatPostMessage`. Replaces the old completion matrix (2026-09-15).
